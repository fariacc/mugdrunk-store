<template>
  <nav class="navbar navbar-expand">
    <div class="container d-block text-right">
      <span class="nav-item text-light mb-0 py-2" v-if="usuario == null || usuario.Permissao != 'admin'" @click="verCarrinho = true">
        <i class="fas fa-shopping-cart" aria-hidden="true"></i>
        {{ items.length }} <span>Itens</span>
      </span>
      <router-link v-if="usuario == null" class="nav-link d-inline-flex" to="/login">
        <span class="nav-item text-light mb-0">
          <i class="fas fa-user" aria-hidden="true"></i> Entrar
        </span>
      </router-link>
      <b-dropdown right v-if="usuario != null" :text="usuario.NomeCompleto">
          ...
          <div v-if="items.length > 0" class="carrinho-modal">
            <ul>
              <li v-for="item in items" class="lista-items">
                <img src="../../assets/images/caneca_caveira.jpg" :alt="item.Descricao">
                <p class="mb-0">
